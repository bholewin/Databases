...
SELECT f.title
FROM film f
LEFT OUTER JOIN (SELECT f.title FROM film f
INNER JOIN film_actor fa ON fa.film_id = f.film_id
INNER JOIN actor a ON a.actor_id = fa.actor_id
WHERE a.first_name = 'SCARLETT' AND a.last_name = 'DAMON') sub ON f.title = sub.title
RIGHT OUTER JOIN (SELECT f.title FROM film f
INNER JOIN film_actor fa ON fa.film_id = f.film_id
INNER JOIN actor a ON a.actor_id = fa.actor_id
WHERE a.first_name = 'BEN' AND a.last_name = 'HARRIS') sub2 ON f.title = sub2.title
